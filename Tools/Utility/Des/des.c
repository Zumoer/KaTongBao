...
void des2DecryptWithKey(unsigned char* input, unsigned char* keys, unsigned char* output)
{    
    int i;
    unsigned char buf[8];
    des3_context ctx3;
    
    des3_set_2keys( &ctx3, keys+0,  keys+8);
    
    for (i=0; i<2; i++) {
        memcpy(buf , (const unsigned char*)input+i*8, 8);
        des3_decrypt( &ctx3, buf, output+i*8);
    }
}

void des2Decrypt(unsigned char* input, unsigned char* output)
{
    unsigned char keys[16];
    memcpy(keys, (unsigned char*)"\x01\x23\x45\x67\x89\xab\xcd\xef\xfe\xdc\xba\x98\x76\x54\x32\x10", 16);
    des2DecryptWithKey(input, keys, output);
}
